...
export function SocialButton({ src, alt, onClick, isDarkModeInvert = false }: SocialButtonProps) {
  ...
  return (
    <button
      className='cursor-pointer hover:scale-105 ease-in-out duration-300 shadow-lg p-2 rounded-lg m-1'
      onClick={onClick}>
      <Image src={src} alt={alt} width={25} height={25} className={imageClassName} />
    </button>
  );
}
